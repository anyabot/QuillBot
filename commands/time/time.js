...
class TTime extends commando.Command {
    ...
    async run(message, input) {
		...
		var dif2 = m3.diff(m)
		var diff2 = humanizeDuration(dif2, { units: ['d', 'h', 'm'] , round: true })
		var mai = false;
		...
		if (date == "Thu") {
			if (m.isBefore(ms)) {
				var dif3 = ms.diff(m)
				var diff3 = humanizeDuration(dif3, { units: ['d', 'h', 'm'] , round: true })
			}
		...
		if (m.isBefore(m4)) {
			var dif4 = m4.diff(m)
			var diff4 = humanizeDuration(dif4, { units: ['d', 'h', 'm'] , round: true })
		}
		else {
			m4 = m4.add(1, 'months')
			var dif4 = m4.diff(m)
		...
		var date4 = words4[0]
		var m5 = moment(year + " " +  month + " " + "01" +" 11:00:00", 'YYYY MMM DD HH:mm:ss')
		var mm5 = m5.format('ddd MMM DD YYYY HH mm ss')
		var words5 = mm5.split(' ')
		var date5 = words5[0]
		if (date5 != "Thu") {
